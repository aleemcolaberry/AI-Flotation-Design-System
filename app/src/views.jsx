...
function AflLogos({ b, swap }) {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
      <span style={{ fontSize: '10.5px', fontWeight: 700, padding: '4px 10px', borderRadius: '999px', flexShrink: 0, background: '#FCF1DD', color: '#8A5A00' }}>New mark proposed this round</span>
      <span style={{ fontSize: '12.5px', color: '#5C6470', maxWidth: '640px' }}>A float held clear of the waterline — systems that keep work moving. One path, reads at 16px.</span>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(300px,100%),1fr))', gap: '14px' }}>
      <div style={tile}><div style={tileTitle}>Mark</div><div style={stage}><span style={mk(72, L.accent)}></span></div><div style={tileMeta}>{src} · min 16px</div></div>
      <div style={tile}><div style={tileTitle}>Horizontal lockup</div><div style={stage}><span style={{ display: 'inline-flex', alignItems: 'center', gap: '14px' }}><span style={mk(44, L.accent)}></span><span style={word(L.fg)}>{b.name}</span></span></div><div style={tileMeta}>Min width 120px · clearspace ½ mark height</div></div>
      <div style={tile}><div style={tileTitle}>Vertical lockup</div><div style={stage}><span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}><span style={mk(72, L.accent)}></span><span style={{ ...word(L.fg), textAlign: 'center', maxWidth: '210px' }}>{b.name}</span></span></div><div style={tileMeta}>Centered contexts — badges, splash, certificates</div></div>
      <div style={tile}><div style={tileTitle}>App icon</div><div style={stage}><span style={{ display: 'inline-flex', alignItems: 'flex-end', gap: '16px' }}>
        {[84, 48, 26].map(s => <span key={s} style={{ width: s + 'px', height: s + 'px', borderRadius: Math.round(s * 0.22) + 'px', background: L.accent, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><span style={mk(Math.round(s * 0.58), L.accentContrast)}></span></span>)}
      ...
      <div style={tile}><div style={tileTitle}>Favicon</div><div style={stage}><span style={{ display: 'inline-flex', alignItems: 'flex-end', gap: '22px' }}>
        {[32, 24, 16].map(s => <span key={s} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}><span style={mk(s, L.accent)}></span><span style={{ fontSize: '10.5px', color: '#8A919B', fontFamily: 'ui-monospace, monospace' }}>{s}px</span></span>)}
      </span></div><div style={tileMeta}>Same SVG, token-colored — still reads at 16px</div></div>
      <div style={tile}><div style={tileTitle}>On dark</div><div style={{ ...stageBase, background: D.bg, border: '1px solid ' + D.line, flex: 1 }}><span style={{ display: 'inline-flex', alignItems: 'center', gap: '14px' }}><span style={mk(44, D.accent)}></span><span style={word(D.fg)}>{b.name}</span></span></div><div style={tileMeta}>{'Dark-mode accent ' + D.accent.toUpperCase() + ' on ' + D.bg.toUpperCase()}</div></div>
      <div style={tile}><div style={tileTitle}>Monochrome</div><div style={stage}><span style={{ display: 'inline-flex', alignItems: 'center', gap: '14px' }}><span style={mk(44, L.fg)}></span><span style={word(L.fg)}>{b.name}</span></span></div><div style={tileMeta}>Single-color contexts — print, stamp, embossing</div></div>
      <div style={tile}><div style={tileTitle}>Usage</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', lineHeight: 1.55, color: '#3A3F46' }}>
          ...
          <div>Colors come from tokens only — accent on bg, fg for mono, accentContrast on accent tiles. Never stretch, rotate or add effects.</div>
  ...
  return <div className={P} style={{ flex: '1 1 400px', minWidth: 'min(360px, 100%)', maxWidth: '600px', background: m.bg, border: '1px solid ' + m.line, borderRadius: '14px', padding: 'clamp(26px, 4vw, 46px) clamp(16px, 3vw, 28px) clamp(18px, 3vw, 26px)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '18px', fontFamily: body, fontSize: base + 'px', color: m.fg, boxSizing: 'border-box', '--dc-dist': mo.enterDistance, animation: A('dcIn') + ' ' + mo.duration.slow + ' ' + mo.easing.enter + ' both', animationDelay: mode === 'dark' ? '70ms' : '0ms' }}>
    ...
    <div style={{ width: '100%', maxWidth: '400px', background: m.bgElevated, border: '1px solid ' + m.line, borderRadius: (b.radius + 6) + 'px', boxShadow: b.shadow, padding: '30px 26px', display: 'flex', flexDirection: 'column', gap: '16px', boxSizing: 'border-box', animation: A('dcPop') + ' ' + mo.duration.slow + ' ' + mo.easing.enter + ' both' }}>
      <div style={{ display: 'flex', justifyContent: 'center', padding: '4px 0 2px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}><span style={aflMask(b.mark, 26, m.accent)}></span><span style={{ fontFamily: heading, fontWeight: 700, fontSize: '19px', color: m.fg }}>{b.name}</span></span>
      </div>
